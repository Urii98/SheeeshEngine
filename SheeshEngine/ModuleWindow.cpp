...
bool ModuleWindow::LoadConfig(JsonParser& node)
{
	...
	if (App->editor->fullscreen) SetFullscreen(App->editor->fullscreen);
	else if (App->editor->fulldesktop) SetFulldesktop(App->editor->fulldesktop);
	if (App->editor->borderless) SetBorderless(App->editor->borderless);
